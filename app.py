...
<head>
    ...
    <style>
        body { font-family: 'Segoe UI', system-ui, sans-serif; background-color: #f4f7f6; padding: 30px; }
        ...
        .remove-btn { position: absolute; top: 5px; right: 5px; width: 22px; height: 22px; background: #ff4d4f; color: white; border-radius: 50%; display: none; justify-content: center; align-items: center; cursor: pointer; border: none; font-weight: bold; }
        .upload-box.active .remove-btn { display: flex; }
        .btn-main { display: block; width: 100%; padding: 15px; background: #1890ff; color: white; border: none; border-radius: 8px; font-size: 16px; font-weight: bold; cursor: pointer; }
...
<div class="container">
    ...
    <p style="font-size: 12px; color: #666; text-align: center;">兼容合并单元格 | 自动追溯首行数据 | 模板历史记忆</p >
    ...
    <div class="upload-section">
        <div class="upload-box" id="boxSource" onclick="document.getElementById('fileSource').click()">
            <button class="remove-btn" onclick="clearFile('source', event)">×</button>
            <b>1. 上传数据源汇总表</b>
            <span id="nameSource" style="font-size:11px; color:#999; margin-top:5px;">支持合并单元格自动填充</span>
            <input type="file" id="fileSource" hidden accept=".xlsx,.xls,.csv">
        </div>
        <div class="upload-box" id="boxTemplate" onclick="document.getElementById('fileTemplate').click()">
            <button class="remove-btn" onclick="clearFile('template', event)">×</button>
            <b>2. 上传空白发货模板</b>
            <span id="nameTemplate" style="font-size:11px; color:#999; margin-top:5px;">决定导出列顺序</span>
...
<script>
    ...
    document.getElementById('fileSource').onchange = (e) => handleFile(e.target.files[0], 'source');
    document.getElementById('fileTemplate').onchange = (e) => handleFile(e.target.files[0], 'template');
    ...
    function handleFile(file, type) {
        ...
        reader.onload = (ev) => {
            ...
            if(type === 'source') { 
                ...
                templateHeaders = XLSX.utils.sheet_to_json(ws, {header: 1})[0]; 
    ...
    function saveTemplate(name, headers) {
        tplHistory = tplHistory.filter(i => i.name !== name); // 避免重复
        tplHistory.unshift({ name, headers });
        if(tplHistory.length > 2) tplHistory.pop(); // 只留2条
    ...
    function checkReady() {
        document.getElementById('processBtn').disabled = !(sourceData && templateHeaders);
    }
